...
basemap = function(az_time){
  ...
  # ama = read_sf('app/shps/Act_Man_Areas.shp') %>% 
  #   st_transform(4326) %>% 
  ...
  leaflet(data = az_spatial) %>%
    ...
    addLegend("bottomright",
              ...
              opacity = 1) %>% 
    addLayersControl(overlayGroups = c("WELLS"), options = layersControlOptions(collapsed = FALSE),
                     baseGroups = c('Topographic', "Relief", 'Satellite'))
}
...
withdrawals_plot <- function(az_time, sector_total, wellid){
  ...
  gg1 = ggplot(subset4, aes(x = sector, y = withdrawal)) +
    geom_col(aes(fill = source), width= 0.5, alpha = 0.8) +
    labs(x = "",
